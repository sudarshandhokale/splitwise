module DeviseTokenAuth
  class OmniauthCallbacksController < DeviseTokenAuth::ApplicationController
    ...
    def redirect_callbacks
      ...
      devise_mapping = request.env['omniauth.params']['resource_class']
    ...
    def auth_origin_url
      ENV['APP_URL'] || omniauth_params['auth_origin_url'] || omniauth_params['origin']
    end
    ...
    def auth_hash
      if !defined?(@_auth_hash)
        if request.env['omniauth.auth'] && request.env['omniauth.auth'].any?
          @_auth_hash ||= request.env['omniauth.auth']
        elsif session['dta.omniauth.auth'] && session['dta.omniauth.auth'].any?
          @_auth_hash ||= session.delete('dta.omniauth.params')
        end
      end
      @_auth_hash
    end
    ...
    def get_resource_from_auth_hash
      ...
      resource = resource_class.where(email: auth_hash['info']['email'])
      if resource.any?
